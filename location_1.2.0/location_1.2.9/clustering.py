...
MAX_GAP      = float(os.environ.get("CLUSTER_MAX_GAP",      60))   # war 20, jetzt 60min → Nacht-Aufenthalte zusammengefasst
...
def merge_adjacent_stays(conn):
    ...
    - weniger als MAX_GAP Minuten auseinanderliegen UND
...
def stays_overlap(conn, start_time, end_time):
    """Prüft ob bereits ein Stay in diesem Zeitraum existiert."""
    cur = conn.cursor()
    cur.execute("""
...
def main():
    ...
    try:
        ...
        print(f"{len(points)} Punkte, R={RADIUS_M}m min={MIN_DURATION}min gap={MAX_GAP}min acc<{MAX_ACCURACY}m")
