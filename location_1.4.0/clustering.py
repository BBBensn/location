...
def merge_adjacent_stays(conn):
    """
    Merged benachbarte Stays die:
    - weniger als MERGE_GAP Minuten auseinanderliegen (default 180min) UND
    - innerhalb von RADIUS_M Metern liegen
    Typischer Fall: zwei "Zuhause" Cluster die durch eine kurze Lücke getrennt sind.
    """
    cur = conn.cursor()
    cur.execute("""
...
def main():
    ...
    try:
        ...
        print(f"{len(points)} Punkte, R={RADIUS_M}m min={MIN_DURATION}min gap={MAX_GAP}min merge_gap={MERGE_GAP}min acc<{MAX_ACCURACY}m")
