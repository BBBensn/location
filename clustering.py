...
Konfigurierbare Parameter (Umgebungsvariablen):
  ...
  CLUSTER_MAX_GAP          Max. Lücke zwischen Punkten in Minuten (default: 60) — initiale Cluster-Bildung
  CLUSTER_MERGE_GAP        Max. Lücke beim Merge benachbarter Stays in Minuten (default: 240)
  CLUSTER_MERGE_GAP_NAMED  Max. Lücke beim Merge wenn beide Stays denselben Namen haben (default: 720 = 12h)
  CLUSTER_MAX_ACCURACY     Max. GPS-Fehler in Metern (default: 80)
...
MAX_GAP           = float(os.environ.get("CLUSTER_MAX_GAP",           60))   # initiale Cluster-Bildung: max. Lücke zwischen Punkten
MERGE_GAP         = float(os.environ.get("CLUSTER_MERGE_GAP",         240))  # Merge-Pass: max. Lücke zwischen benachbarten Stays
MERGE_GAP_NAMED   = float(os.environ.get("CLUSTER_MERGE_GAP_NAMED",   720))  # Merge-Pass für benannte Stays (tagesübergreifend, z.B. Zuhause)
MAX_ACCURACY      = float(os.environ.get("CLUSTER_MAX_ACCURACY",       80))  # Punkte mit GPS-Fehler >80m werden ignoriert
...
def build_clusters(points):
    ...
    current = [points[0]]
    for p in points[1:]:
        # Ausreißer ignorieren: GPS-Genauigkeit zu schlecht
        if p.get("accuracy") and float(p["accuracy"]) > MAX_ACCURACY:
            continue
        last = current[-1]
...
def merge_adjacent_stays(conn):
    """
    Merged benachbarte Stays die nah genug auseinanderliegen UND innerhalb von RADIUS_M Metern liegen.
    Gap-Schwelle: MERGE_GAP_NAMED wenn beide Stays denselben nicht-leeren Namen tragen (z.B. "Zuhause"
    über Nacht), sonst MERGE_GAP.
    """
    cur = conn.cursor()
    cur.execute("""
        SELECT id, start_time, end_time, latitude, longitude,
               name, city, district, country, point_count,
               avg_temperature, weather_desc
        FROM location_stays
    ...
    while i < len(stays) - 1:
        ...
        # Zeitlücke zwischen den beiden Stays
        gap = (b["start_time"] - a["end_time"]).total_seconds() / 60
        both_named = bool(a.get("name") and b.get("name") and a["name"] == b["name"])
        gap_limit = MERGE_GAP_NAMED if both_named else MERGE_GAP
        if gap > gap_limit:
            i += 1
            continue
        ...
        # Mergen: a absorbiert b
        new_end      = b["end_time"]
        new_duration = (new_end - a["start_time"]).total_seconds() / 60
        new_count    = (a["point_count"] or 0) + (b["point_count"] or 0)
        # Neues Zentrum (gewichteter Durchschnitt nach Punktanzahl)
        wa = a["point_count"] or 1
        wb = b["point_count"] or 1
        new_lat = (float(a["latitude"])*wa + float(b["latitude"])*wb) / (wa+wb)
        new_lon = (float(a["longitude"])*wa + float(b["longitude"])*wb) / (wa+wb)
        # Name: bevorzuge gesetzten Namen
        new_name = a["name"] or b["name"]

        cur.execute("""
            UPDATE location_stays
            SET end_time = %s, duration_minutes = %s, point_count = %s,
                latitude = %s, longitude = %s, name = %s, updated_at = NOW()
            WHERE id = %s
        """, (new_end, round(new_duration,1), new_count,
              round(new_lat,7), round(new_lon,7), new_name, a["id"]))
        cur.execute("DELETE FROM location_stays WHERE id = %s", (b["id"],))
        conn.commit()

        # b aus Liste entfernen, a updaten
        stays[i] = {**a, "end_time": new_end, "duration_minutes": new_duration,
                    "point_count": new_count, "latitude": new_lat, "longitude": new_lon,
                    "name": new_name}
        stays.pop(i+1)
        merged += 1
        # Nicht i erhöhen — nächsten Stay gegen das neue a prüfen

    return merged


def get_overlapping_stay(conn, start_time, end_time):
    """Gibt den ersten Stay zurück der mit [start_time, end_time] überlappt."""
    cur = conn.cursor()
    cur.execute("""
        SELECT id, start_time, end_time, latitude, longitude, point_count
        FROM location_stays
...
def main():
    ...
    try:
        ...
        print(f"{len(points)} Punkte, R={RADIUS_M}m min={MIN_DURATION}min gap={MAX_GAP}min merge_gap={MERGE_GAP}min merge_gap_named={MERGE_GAP_NAMED}min acc<{MAX_ACCURACY}m")
        ...
        for cluster in clusters:
            ...
            if stay["duration_minutes"] < MIN_DURATION:
                skipped += 1
                continue
            existing = get_overlapping_stay(conn, stay["start_time"], stay["end_time"])
            if existing:
                # Gleicher Ort + neuer Cluster reicht weiter → Stay verlängern
                if existing["latitude"] and existing["longitude"] and stay["end_time"] > existing["end_time"]:
                    dist = haversine_m(
                        float(existing["latitude"]), float(existing["longitude"]),
                        stay["latitude"], stay["longitude"]
                    )
                    if dist <= RADIUS_M:
                        new_end = stay["end_time"]
                        new_duration = (new_end - existing["start_time"]).total_seconds() / 60
                        new_count = (existing["point_count"] or 0) + stay["point_count"]
                        cur.execute("""
                            UPDATE location_stays
                            SET end_time = %s, duration_minutes = %s, point_count = %s, updated_at = NOW()
                            WHERE id = %s
                        """, (new_end, round(new_duration, 1), new_count, existing["id"]))
                        conn.commit()
                        extended += 1
                        label = stay["name"] or (f"{stay['district']}, {stay['city']}" if stay["district"] else stay["city"] or "?")
                        print(f"  [+] {existing['start_time'].strftime('%H:%M')}–{new_end.strftime('%H:%M')} "
                              f"({round(new_duration)}min) @ {label}")
                    else:
                        skipped += 1
                else:
                    skipped += 1
        ...
        print(f"Fertig: {inserted} neu, {extended} verlängert, {skipped} übersprungen.")

        # Merge-Pass: benachbarte Stays am gleichen Ort zusammenführen
        merged = merge_adjacent_stays(conn)
        if merged:
            print(f"Merge: {merged} Stays zusammengeführt.")
    finally:
        conn.close()
